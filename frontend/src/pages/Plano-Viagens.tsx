...
const PlanoViagens: React.FC = () => {
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:3000/PlanoViagem/${userId}`)
      ...
      .then((data) => {
        const now = new Date();
        const upcoming = data.filter((plan: any) => new Date(plan.dataInicio) > now);
        const history = data.filter((plan: any) => new Date(plan.dataInicio) <= now);

        const formatPlan = (plan: any, faded = false) => ({
          title: plan.nome,
          ...
          daysLeft: Math.max(0, Math.ceil((new Date(plan.dataInicio).getTime() - now.getTime()) / (1000 * 60 * 60 * 24))),
          faded
        });

        setTravelPlans(upcoming.map((p: any) => formatPlan(p)));
        setTravelHistory(history.map((p: any) => formatPlan(p, true)));
      })
      .catch((err) => {
